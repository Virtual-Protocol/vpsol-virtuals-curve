...
#[wasm_bindgen]
#[derive(Debug)]
pub struct ConstantProduct(virtuals_curve::ConstantProduct);
...
impl ConstantProduct {
    ...
    pub fn init(
        ...
        fee: u16,
        precision: Option<u8>,
    ) -> Result<ConstantProduct, JsError> {
        // Assert non-zero values of X and Y
        Ok(Self(virtuals_curve::ConstantProduct::init(
            x, y, fee, precision,
        )?))
    }
    ...
    pub fn k_from_xy(x: u64, y: u64) -> Result<JsValue, JsError> {
        let k = virtuals_curve::ConstantProduct::k_from_xy(x, y)?;
        Ok(JsValue::from(k))
    }
    ...
    pub fn spot_price_from_pair(x: u64, y: u64, precision: u32) -> Result<SpotPrice, JsError> {
        let v = virtuals_curve::ConstantProduct::spot_price_from_pair(x, y, precision)?;
        Ok(SpotPrice {
            lower: (v.amount & 0b00000000000000001111111111111111) as u64,
            upper: (v.amount >> 64) as u64,
    ...
    pub fn x2_from_y_swap_amount(x: u64, y: u64, a: u64) -> Result<JsValue, JsError> {
        Ok(virtuals_curve::ConstantProduct::x2_from_y_swap_amount(x, y, a)?.into())
    }

    #[wasm_bindgen]
    pub fn y2_from_x_swap_amount(x: u64, y: u64, a: u64) -> Result<JsValue, JsError> {
        Ok(virtuals_curve::ConstantProduct::x2_from_y_swap_amount(y, x, a)?.into())
    }

    #[wasm_bindgen]
    pub fn delta_x_from_y_swap_amount(x: u64, y: u64, a: u64) -> Result<JsValue, JsError> {
        Ok(virtuals_curve::ConstantProduct::delta_x_from_y_swap_amount(x, y, a)?.into())
    }

    #[wasm_bindgen]
    pub fn delta_y_from_x_swap_amount(x: u64, y: u64, a: u64) -> Result<JsValue, JsError> {
        Ok(virtuals_curve::ConstantProduct::delta_x_from_y_swap_amount(y, x, a)?.into())
    }
    ...
    pub fn swap_unsafe(&mut self, p: LiquidityPair, a: u64) -> Result<SwapResult, JsError> {
        let p = match p {
            LiquidityPair::X => virtuals_curve::LiquidityPair::X,
            LiquidityPair::Y => virtuals_curve::LiquidityPair::Y,
        };
